...
class QuickOrderList extends HTMLElement {
  constructor() {
    ...
    const form = this.querySelector(`form#QuickOrderList-${this.quickOrderListId}`);
    ...
    form.addEventListener('change', debouncedOnChange.bind(this));
  }

  cartUpdateUnsubscriber = undefined;

  getSectionsUrl() {
    return window.quickOrderSearchEnabled
      ? `/search?q=${encodeURIComponent(window.quickOrderSearchQuery)}`
  ...
  renderSections(parsedState, sectionsToRender = this.getSectionsToRender()) {
